...
//            generateQRCode()
    ...
    fun generateQRCode(){
        val bitmap = encodeAsBitmap(edt_input.text.toString(),300,300,context)
    ...
    fun createImageFile(bitmapScaled : Bitmap?){
        ...
        val filepath = Environment.getExternalStorageDirectory().absolutePath + File.separator + edt_input.text.toString()+".png"
        val f = File(filepath)
        f.createNewFile()
